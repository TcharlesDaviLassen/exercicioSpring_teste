document.addEventListener("DOMContentLoaded", () => {
  ...
  //  const elementIdUserEdit = document.getElementById('idUserEdit')[i];
  //  const idUserEdit = elementIdUserEdit.getAttribute('data-idUserEditData')[i];
  ...
  // Adicione um ouvinte de eventos de clique às linhas da tabela
  ...
  function updateSecondaryTable(searchTerms, conn) {
    ...
    const numeroIsEmpty = searchTerms[1] === "" || searchTerms[1] === null;
    const emailIsEmpty = searchTerms[2] === "" || searchTerms[2] === null;
    const dataIsEmpty = searchTerms[3] === "" || searchTerms[3] === null;

    if (nomeIsEmpty && numeroIsEmpty && emailIsEmpty && dataIsEmpty) {
      ...
      rows.forEach(row => {
        ...
        const searchTermNumero = searchTerms[1].toLowerCase();
        const searchTermEmail = searchTerms[2].toLowerCase();
        const searchTermData = searchTerms[3].toLowerCase();

        if (
          (!searchTermNome || nome.toLowerCase().includes(searchTermNome)) &&
          (!searchTermNumero || numero.toLowerCase().includes(searchTermNumero)) &&
          (!searchTermEmail || email.toLowerCase().includes(searchTermEmail)) &&
          (!searchTermData || data.toLowerCase().includes(searchTermData))
        ) {
  ...
  updateSecondaryTable(initialSearchTerms, 0);
});
